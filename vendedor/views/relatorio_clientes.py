# vendedor/views/relatorio_clientes.py - COM FILTRO DE CLIENTE
...
def relatorio_clientes(request):
   ...
   if request.GET.get('exportar_excel') and dados_relatorio:
       return exportar_relatorio_excel(dados_relatorio, meses_periodo, {
           ...
           'query': query if query else 'Todos',
           'incluir_coligados': incluir_coligados,
           'apenas_com_vendas': apenas_com_vendas,
...
def gerar_dados_relatorio_com_acesso(user, data_inicio, data_fim, loja_codigos, vendedor_codigos, estados, 
                                   grupo_codigos, fabricante_codigos, produto_codigos, query, incluir_coligados, apenas_com_vendas):
...
def exportar_relatorio_excel(dados_relatorio, meses_periodo, filtros):
   ...
   if filtros['query'] and filtros['query'] != 'Todos':
       ws[f'A{linha_atual}'] = "Cliente:"
       ws[f'B{linha_atual}'] = filtros['query']
       linha_atual += 1
